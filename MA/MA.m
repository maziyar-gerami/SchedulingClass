...
nSlots = 4;         % Number of slot times per day
...
MaxIt=11;      % Maximum Number of Iterations
...
empty_individual.j_minusScore=[];
empty_individual.B_minusScore=[];
...
while (it<12)
    ...
    for i=1:length(pop)
        ...
        P=value/sum(value);
        % Local Improver
        poplocalImpr=repmat(empty_individual,2,1);
        %poplm=repmat(empty_individual,localImprPopSize,1);
        poplm=[];
        poplmt=repmat(empty_individual,2,1);
        if ~mod(it, eta)
            ...
            [poplmt(itm,1).Table, poplmt(itm,2).Table]=Crossover(p1.Table,p2.Table, courseTable, teacherSlotTable, nCourses);
        
            [poplmt(itm,1).E_minusScore, poplmt(itm,1).P_minusScore,poplmt(itm,1).S_minusScore, poplmt(itm,1).j_minusScore,poplmt(itm,1).B_minusScore] = Objective(poplmt(itm,1).Table,courseTypes,nDays, nSlots);
        
            [poplmt(itm,2).E_minusScore, poplmt(itm,2).P_minusScore,poplmt(itm,2).S_minusScore, poplmt(itm,1).j_minusScore,poplmt(itm,1).B_minusScore] = Objective(poplmt(itm,2).Table,courseTypes,nDays, nSlots);
                ...
                for k=1:2
                    ...
                    poplocalImpr(k).Table=Mutate(p.Table, courseTable, teacherSlotTable, nCourses);

                    % Evaluate Mutant
                    [poplocalImpr(k).E_minusScore, poplocalImpr(k).P_minusScore,poplocalImpr(k).S_minusScore, popm(k).j_minusScore,popm(k).B_minusScore] = Objective(poplocalImpr(k).Table,courseTypes,nDays, nSlots);
                
                    poplm=[poplm; poplocalImpr(k)]; 
